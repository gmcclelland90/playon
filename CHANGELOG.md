...
- **Venice mid-size two-step tool loop** — After the first `servers_list`, mid-size Venice models (`qwen3-5-9b`, `mistral-small-3-2-24b-instruct`) were aborting the chat loop on empty assistant content (or a one-step prose answer to a numbered two-step request). The orchestrator now continues once, tells the model the host loops after each tool, offers `servers_get` on the install/maintain catalog, and echoes `name` on tool result messages (`#980`).
- **The Isle SteamCMD fetch** — catalog `games.the-isle` 0.1.1 documents `-beta evrima` but ships no SteamCMD beta field, and Home only applied `steamBetaLinux` on Linux. The skill is Windows-only, so matrix install fetched the abandoned Legacy `public` branch (`fetch failed` / empty depot). Home now has cross-platform `steamBeta` and pins The Isle to `evrima` (`#965`).
- **Stormworks UDP 25564** — Steam tool `1247090` is a discontinued stub that starts and never binds. PlayOn remaps that app id to client `573090`, writes jail `start.bat` + `server_data/server_config.xml` with `+server_dir` (not `%APPDATA%\Stormworks`), and locates `server64.exe` in the jail, `PLAYON_MANAGED_FROM`, or a host Steam library. Lab matrix keeps going after anonymous `no_subscription` so a Windows client install can still bind (`#959`).
- **Enshrouded join / `port_open`** — catalog `games.enshrouded` 0.1.2 still advertises UDP `game` 15636, but Keen CU2+ dedicated builds bind only `queryPort` 15637 (they delete `gamePort` from `enshrouded_server.json`). Home remaps that stale game port to 15637 and sets `queryPortName=query` so matrix requires the socket that actually listens. Writing `gamePort` in JSON does not bring 15636 back (`#957`).
