...
describe("classifySkillSource", () => {
  ...
  it("classifies platform, installed, draft, and fixture paths", () => {
    expect(
      classifySkillSource(
        entry("platform.docker-basics", path.join("/opt/playon/packages/platform/docker-basics")),
        dataRoot,
      ),
    ).toBe("platform");
    ...
    expect(
      classifySkillSource(
        entry("fixtures.lab-docker-server", path.join("/opt/playon/packages/fixtures/lab")),
        dataRoot,
      ),
    ).toBe("fixture");
  });
