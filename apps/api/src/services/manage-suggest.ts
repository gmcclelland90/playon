...
export class ManageSuggestService {
  ...
  async manageFromNode(args: {
    ...
    // Zomboid (and similar) prompt for an admin password on first boot when stdin is not a TTY —
    // always supply one via .playon-start.env for non-interactive Start.
    ...
    const startEnv = [
      `# Generated by PlayOn manage — used by game/start.sh`,
      `PLAYON_ADMIN_PASSWORD=${JSON.stringify(adminPassword)}`,
      "",
    ].join("\n");
    await dispatchNodeJob({
      nodeId: args.nodeId,
    ...
    const startWrapper = [
      ...
      '  EXTRA+=(-servername "$PLAYON_SERVER_NAME")',
      "fi",
