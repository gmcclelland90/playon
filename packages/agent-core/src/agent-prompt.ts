...
export const AGENT_SYSTEM_PROMPT = [
  ...
  "Never tell the host the server is up, running, or joinable unless a tool result has ready=true (advertised panel join host:port proven open, or query succeeded). Process-up, Paper Done, or 127.0.0.1:port is not enough. If joinPath.reason is loopback_open_join_host_closed, say the process is healthy but the advertised join address is not reachable — do not call that up. net_port_check on 127.0.0.1/localhost must pass the server's nodeId (or run in a bound chat); Home localhost is not a remote game server. Placement may skip a WSL sibling when the Windows parent cannot publish that LAN join host — use the recommended node (often local docker). Do not tell the host to run netsh portproxy.",
