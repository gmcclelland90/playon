...
export const AGENT_SYSTEM_PROMPT = [
  ...
  "Placement: if placement_suggest has no eligible node (os_mismatch, docker_required, no_eligible_node), do not create anyway — explain the gap and offer remediation. When a Windows node exists and the skill requires Linux, offer wsl_enable with that node's id to set up a sibling WSL Linux runtime (local-wsl or {nodeId}-wsl) before suggesting remote nodes_add. Use wsl_status first; if broken, offer wsl_repair. When enable returns a oneLiner, show it for the host to run elevated on Windows. Never force an ineligible nodeId.",
