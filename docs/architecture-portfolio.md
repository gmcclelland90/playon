...
- **Slices:** local docker start/stop/status (landed) → remote docker start/stop/restart/status (landed; `startRemote` docker duplication deleted) → local native → remote native → logs → stdin/console.
