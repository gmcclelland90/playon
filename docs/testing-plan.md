...
- Managed instance liveness (leftover reap, healthy reuse, port-dead, health-restart single-instance, workshop_update notify-only) is unit-covered in `@playon/shared` + `@playon/runtime` + api runtime-handle / health / watcher tests ([#880](https://github.com/gmcclelland90/playon/issues/880))
